...
class EditButtonFormatter extends React.Component {
    render(){
        const value = this.props;
        return (<div><Button>Edit</Button> <Button>Review</Button></div>);
...
class NumberFormatter extends React.Component {
    render() {
        ...
        return (<span style={{textAlign: 'right'}}><div title={value}>{value}</div></span>);
    }
}
...
export default class SwzGridView extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            pageSize: 20,
            selectedIndexes: [],
            rowsCount: SwzGridView.staticIsServerMode(props)? undefined : 0
        };
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if (!jsonEqual(prevState.filter, nextProps.filter)) {
            ...
            if (!SwzGridView.staticIsServerMode(nextProps)) {
                const newItems = Array.isArray(nextProps.value) ? SwzGridView.staticGetSortedAndFilteredItems(nextProps.value, nextProps.filter,
            ...
            if (!SwzGridView.staticIsServerMode(nextProps) && prevState.originalItems !== nextProps.value) {
                newState = {};
                const newItems = Array.isArray(nextProps.value) ? SwzGridView.staticGetSortedAndFilteredItems(nextProps.value, prevState.filter,
                    prevState.sort, nextProps.defaultSort) : [];
                newState.items = newItems;
                newState.rowsCount = newItems.length;
                newState.selectedIndexes = [];
    ...
    isServerMode() {
        return SwzGridView.staticIsServerMode(this.props);
    }
    ...
    refresh() {
        if (this.isServerMode()) {
            ...
            const newItems = Array.isArray(this.props.value) ? SwzGridView.staticGetSortedAndFilteredItems(this.props.value, this.state.filter,
    ...
    render() {
        ...
        if (this.props.autoHeight) {
            ...
            className = (className === undefined ? "" : (className + " ")) + "clover-swzgridview-autoHeight";
        }
        else {
...
//        }
    ...
    getColumns() {
        ...
        columns.forEach(function (item) {
            ...
            else if (item.type === "swzbuttons"){
                item.formatter = EditButtonFormatter;
            }
    ...
    handleGridSort(sortColumn, sortDirection) {
        ...
        if (pagerType === "server") {
            ...
            stateDelta.items = SwzGridView.staticGetSortedAndFilteredItems(this.props.value,
    ...
    handleFilterChange(filter) {
        ...
        if (SwzGridView.staticIsServerMode(this.props)) {
            stateDelta.items = [];
            stateDelta.selectedIndexes = [];
...
class RowLoadingRenderer extends React.Component {
    ...
    getClassName() {
        return this.props.row.__loading ? 'clover-swzgridview-rowloading' : "";
    };
