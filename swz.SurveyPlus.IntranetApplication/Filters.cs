...
namespace swz.SurveyPlus.IntranetApplication
{
    public class Filters : IServerActionsProvider
    {
        ...
        {
            var userId = CloverRuntime.Security.CurrentUser.ImpersonatedUserId.HasValue
            ...
            var currentUserInbox = (await inboxModel.GetAsync(Filter.And.Equal(userId.ToString(), "IdentityId")))
                .Select(e => (Guid) (e as dynamic).ProcessId).ToList();
        ...
        public async Task<Filter> Outbox(EntityModel model, List<dynamic> entities, dynamic options)
        {
            ...
            var currentUserOutbox =
                (await historyModel.GetAsync(Filter.And.Equal(userId.ToString(), "ExecutorIdentityId")))
                .Select(e => (Guid) (e as dynamic).ProcessId).ToList();
