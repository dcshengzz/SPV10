...
namespace swz.SurveyPlus.IntranetApplication
{
    public static class ClientNotifiers
    {
        public static async Task NotifyClientsAboutInboxStatus(string userId)
        {
            ...
            var historyModel =
                await MetadataToModelConverter.GetEntityModelByModelAsync("WorkflowProcessTransitionHistory");
            var inboxCount = string.IsNullOrWhiteSpace(userId)
                ? 0L
                : await inboxModel.GetCountAsync(Filter.And.Equal(userId, "IdentityId"));

            var outboxCount = 0L;
            if (!string.IsNullOrWhiteSpace(userId))
            {
                var outboxProcessIds = (await historyModel.GetAsync(Filter.And.Equal(userId, "ExecutorIdentityId")))
                    .Select(e => (Guid) (e as dynamic).ProcessId).Distinct();
        ...
        public static async Task NotifyClientsAboutInboxStatus(List<string> userIds)
        {
            ...
            foreach (var id in userIds)
            {
                ...
                var outboxCount = outboxes.Where(i => (i as dynamic).ExecutorIdentityId == id)
                    .Select(i => (Guid) (i as dynamic).ProcessId).Distinct().Count();
        ...
        {
            await CloverRuntime.SendStateChangeToUserAsync(userId, "app.extra", new Dictionary<string, object>
            {
                {"inbox", inboxCount},
                {"outbox", outboxCount}
            });
        ...
        public static async Task SpNotifyClients(string userId, object spData)
        {
            await CloverRuntime.SendStateChangeToUserAsync(userId, "app.extra", new Dictionary<string, object>
            {
                {"spData", spData}
        ...
        public static async Task DeleteWokflowAndNotifyClients(EntityModel model, List<ChangeOperation> changes)
        {
            ...
            usersToNotify =
                usersToNotify.Distinct().Where(s => s != null)
                    .ToList(); //ExecutorIdentityId can be null for timer transitions
