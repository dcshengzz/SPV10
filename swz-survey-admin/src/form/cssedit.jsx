...
export default class CloverAdminCssEdit extends BaseComponent {
  constructor(props) {
    super(props);
  ...
  redrawEditor(){
      if($("#clover-admin-formlogic-editcode").length > 0){
        ...
        var sourcecode = this.state.form.cssCode;
        if(sourcecode == undefined)
          sourcecode = " ";
        this.editor.setValue(this.state.form.cssCode, -1);
      }
  }

  applyStyleToEditor(cssStyle){
    var editor = ace.edit("clover-admin-formlogic-editcode");
    this.editor.setValue(editor.getValue() + '\n\n' + cssStyle, -1);
  }
  ...
  render(){
    ...
    return (<div>
        <Breadcrumb>
            <Breadcrumb.Section onClick={this.props.parent.back.bind(this.props.parent)} link>Form style</Breadcrumb.Section>
            <Breadcrumb.Divider />
            <Breadcrumb.Section active>{this.props.data.name}</Breadcrumb.Section>
            <Breadcrumb.Divider icon='right angle' />
            <Breadcrumb.Section href={"?apanel=forms&aid=" + this.state.id} onClick={this.openBuilder.bind(this)}>Forms</Breadcrumb.Section>
            <Breadcrumb.Divider />
            <Breadcrumb.Section  href={"?apanel=formlogic&aid=" + this.state.id} onClick={() => this.props.showConfirm(() => this.openFormLogic(this) )}>Form logic</Breadcrumb.Section>
            {/*<Breadcrumb.Divider />
 ...
 */}        </Breadcrumb>
        <div className="cloveradmin-toolbar" style={{marginTop:"5px"}}>
            ...
            applyActionHandler={this.applyStyleToEditor.bind(this)} />
  ...
  onSave(){
    var editor = ace.edit("clover-admin-formlogic-editcode");
    this.state.form.CssCode = editor.getValue();
    ...
    this.state.form.__type = "csscode";
