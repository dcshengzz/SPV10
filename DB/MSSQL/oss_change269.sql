...
[UpdatedBy]='b9d69ba9-282b-d3d2-8f23-efc2596a082c', [UpdatedDate]='2021-12-30 12:49:41.880', 
[Data]=N'{
    init: function (args) {
        //--------------------------------------------
        ...
        const statusTitleFormatter  = function (p) {
            const statusBtn = CloverApp.API.createElement("button", { onClick: () => dataeditordeploymentUserActions.showStatusModal(innerArgs, p.row.Id), className: "ui button mini secondary invert" }, p.value);
            
            const elements = [];
            
            if(p.row.UID !== "swzanonymous"){elements.push(exemptFormatter(p));}
            
            elements.push(resetFormatter(p));
            
            if(p.row.UID !== "swzanonymous"){elements.push(rejectFormatter(p));}
        ...
        const initialResponseFormatter  = function (p) {
            const elements = [];
            const originLabel = "ui label tiny";
            if(p.row.InitialResponseAs !== "Unknown" && p.row.InitialResponseAs !== "PrePopulated" && p.row.InitialResponseAs !== null){
                const iniResponseAs = CloverApp.API.createElement("label", { className: originLabel }, p.row.InitialResponseAs);
                const iniResponseAsDiv = CloverApp.API.createElement("div", {style:{marginTop:"2px"} },iniResponseAs );
                elements.push(iniResponseAsDiv);
            }
            
            if(p.row.InitialResponseBy !== "Unknown" && p.row.InitialResponseBy !== null){
                const iniResponseBy = CloverApp.API.createElement("label", {  className: originLabel, style:{marginTop:"2px"} }, p.row.InitialResponseBy);
                const iniResponseByDiv = CloverApp.API.createElement("div", {style:{marginTop:"2px"} },iniResponseBy );
                elements.push(iniResponseByDiv);
            }
            
            if(p.row.InitialResponseVia !== "Unknown" && p.row.InitialResponseVia !== null){
                const iniResponseVia = CloverApp.API.createElement("label", { className: originLabel }, p.row.InitialResponseVia);
                const iniResponseViaDiv = CloverApp.API.createElement("div", {style:{marginTop:"2px"} },iniResponseVia );
                elements.push(iniResponseViaDiv);
            }
            return elements;
