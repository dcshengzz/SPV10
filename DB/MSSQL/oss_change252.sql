...
[UpdatedBy]='b9d69ba9-282b-d3d2-8f23-efc2596a082c', [UpdatedDate]='2021-12-07 12:20:06.787', 
[Data]=N'{
    init: function (args) {
        //--------------------------------------------
        ...
        const initialResponseFormatter  = function (p) {
            ...
            if(p.row.InitialResponseAs !== "Unknown" && p.row.InitialResponseAs !== "PrePopulated" && p.row.InitialResponseAs !== null){
                const iniResponseAs = CloverApp.API.createElement("button", { className: "ui button mini compact" }, p.row.InitialResponseAs);
                elements.push(iniResponseAs);
            }
            
            if(p.row.InitialResponseBy !== "Unknown" && p.row.InitialResponseBy !== null){
                const iniResponseBy = CloverApp.API.createElement("button", {  className: "ui button mini compact" }, p.row.InitialResponseBy);
                elements.push(iniResponseBy);
            }
            
            if(p.row.InitialResponseVia !== "Unknown" && p.row.InitialResponseVia !== null){
                const iniResponseVia = CloverApp.API.createElement("button", { className: "ui button mini compact" }, p.row.InitialResponseVia);
