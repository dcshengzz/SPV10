/**
 * JQuery plugin to display radio buttons as a slider
 */
(function($) {
    'use strict';
    ...
    Plugin.prototype.addBase = function() {
        ...
        $inputs.each(function() {
			var $item;
			if($(this).hasClass('RadioGroupHide')){
				$item = $('<div class="' + options.itemClass + ' RadioGroupHide" id="rs_' + this.id + '">');
			} else {
				$item = $('<div class="' + options.itemClass + '" id="rs_' + this.id + '">');
			}
            $(this)
                .nextUntil('input')
                .addBack()
    ...
    Plugin.prototype.setSlider = function() {
        ...
        if ($inputChecked.length > 0) {
            ...
            if (this.orientation === 'vertical') {
				...
				if(fillDimension === 10.5){
					console.log("printmode");
					$fill[0].style[this.DIMENSION] = (currentValue / this.$inputs.length * 100) - (1 / this.$inputs.length * 100 / 2) + '%';
					$handle[0].style[this.DIRECTION_STYLE] = (currentValue / this.$inputs.length * 100) - (1 / this.$inputs.length * 100 / 2) + '%';
				}
				else {
